...
class SYSAction
{
     ...
     */
    public static function MIKI_sms_send($c, $p)
    {
        //global $MIKI_sms_username;
        //global $MIKI_sms_password;
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'http://api.message.net.tw/send.php?id=' . '0910653517' . '&password=' . 'Astrawm232' . '&tel=' . $p . '&msg=' . urlencode($c) . '&encoding=utf8');
    ...
    {
        return date("YmdHis") . substr(uniqid(mt_rand(), true), 0, 8);
    }
...
class UserAction
{
    ...
    public static function COM_Login($loginUsername, $password, $RememberMe, $sha512 = true)
    {
        if (isset($loginUsername)) {
            ...
            MYPDO::$join = [
                'sys_company_group' => [
                    'company_group_id', 'sys_company_group.id', 'LEFT JOIN', '='
                ]
            ];
            MYPDO::$where = [
                'account' => $loginUsername,
            ...
            MYPDO::$field = 'account,sys_company.switch,date_deadline,sys_company.id,company_group_id';
            $row = MYPDO::first();
            ...
            if ($row['account'] != "") {
                ...
                if ($row['switch'] == 'OFF') {
                    ...
                    BaseWork::cookie('Msg', '此帳號停用', 1);
                    header("Location:login.php");
                } elseif ($row['date_deadline'] < date('Y-m-d')) {
                    //群組帳號到期
                    BaseWork::cookie('Msg', '此帳號已到期', 1);
                    header("Location:login.php");
                } elseif ($row['date_deadline'] == null) {
                    //帳號期限無限制
    ...
    /**
     * *將欲上傳圖片按照等比例進行縮圖並維持透明背景
     * 注意:此縮圖方式只支援png、gif
     * @param string $from_filename 原始路徑
     * @param string $save_filename 縮圖後存放路徑
     * @param int $new_width 圖寬
     * @param int $new_height 圖高
     * @return false|mixed
     */
    public static function image_resize_transparent($from_filename, $save_filename, $new_width = 256, $new_height = 256)
    {
        // 透明背景只有 png 和 gif 支援
    ...
    /**
     * 根據條件取得下層資料ˋ
     * @param string $selectColName 取得資料欄位名稱
     * @param string $tableName 資料表名稱
     * @param string $whereColName 篩選名稱
     * @param string $whereColValue 篩選值
     */
...
//        // 獲取父節點下的所有子節點
//        MYPDO::$table = $tableName;
//        MYPDO::$where = [
//            $whereColName => $whereColValue
//        ];
//        $result = MYPDO::first();
//        // 顯示每個子節點
//        for($i=0;$i<10;$i++) {
//            if ($result[$selectColName] != '') {
//                $array_data[] = $result[$selectColName];
//                // 遞迴呼叫當前函式，顯示再下一級的子節點
//                self::getChildren($selectColName, $tableName, $whereColName, $result[$selectColName]);
//            }else{
//                break;
//            }
//
...
class Email
{
    /**
     * 發送email
     * @param array $email 欲傳送之email
     * @param array $params message:通知訊息，vm_name:機器名稱，location:位置，time:發生時間
     * @param string $action vm:智販機事件，accuont、vm_time_out:帳號或智販機到期事件
     */
    public static function SendMail($email = [], $params = [], $action)
    {
        $url = 'https://aladdin.eng-astra.com.tw/service/AstraSendMail.php';
        $myvars = 'email=' . serialize($email) . '&params=' . serialize($params) . '&action=' . $action;
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); //跳過SSL檢查
